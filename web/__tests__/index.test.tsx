...
describe("Home Component", () => {
  ...
  it("renders the capture form", () => {
    ...
    // The UI shows a "Capture Duration:" label and a separate "seconds" hint
    expect(screen.getByText(/capture duration/i)).toBeInTheDocument();
    expect(screen.getByText(/seconds/i)).toBeInTheDocument();
    expect(screen.getByText(/output filename/i)).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /start capture/i })
    ).toBeInTheDocument();
  });
  ...
  it("updates filename input", async () => {
    ...
    const filenameInput = screen.getByDisplayValue(
      "packet_capture.csv"
    ) as HTMLInputElement;
  ...
  it("shows loading state during capture", async () => {
    ...
    await waitFor(() => {
      ...
      const capturingButton = buttons.find((btn) =>
  ...
  it("displays error message on capture failure", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/failed to capture packets/i)
      ).toBeInTheDocument();
    });
  });
